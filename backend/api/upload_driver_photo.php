...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['photo']) && isset($_POST['driver_id'])) {
        $driverId = $_POST['driver_id'];
        $file = $_FILES['photo'];
        
        // Klasör kontrolü
        $targetDir = "../uploads/driver_profiles/";
        if (!file_exists($targetDir)) {
            mkdir($targetDir, 0777, true);
        }
        
        $fileName = "driver_" . $driverId . "_" . time() . "_" . basename($file["name"]);
        ...
        if (in_array(strtolower($fileType), $allowTypes)) {
            if (move_uploaded_file($file["tmp_name"], $targetFilePath)) {
                ...
                $dbFilePath = "backend/uploads/driver_profiles/" . $fileName;
                
                try {
                    $stmt = $pdo->prepare("UPDATE drivers SET profile_photo = ? WHERE id = ?");
                    $stmt->execute([$dbFilePath, $driverId]);
        ...
        $response['success'] = false;
        $response['message'] = 'Dosya veya sürücü ID eksik.';
    }
} else {
    $response['success'] = false;
